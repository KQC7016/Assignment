...
def para_over_time(input_dataset, x_colum, y_colum, number: int):
    ...
    # data = input_dataset.set_index('Datetime', inplace=True)
    # data.set_index('Datetime', inplace=True)
    # hourly_mean = data.resample('Hour').mean()
    # daily_mean = data.resample('Day').mean()
    # monthly_mean = data.resample('Month').mean()
...
def ape_mape(absolute_percent_errors, mean_absolute_percent_errors):
    plt.ioff()

    # 绘制 APE 和 MAPE
    print(f"\n[log_14] Plot Absolute Percent Errors & Mean Absolute Percent Errors")
    plt.figure(figsize=(20, 10))
    plt.plot(absolute_percent_errors, label='Absolute Percent Error')
    plt.axhline(y=mean_absolute_percent_errors, color='r', linestyle='--', label=f'MAPE: {mean_absolute_percent_errors:.4f}')
    plt.xlabel('Time Step')
    plt.ylabel('Absolute Percent Error')
    plt.title('Absolute Percent Error and MAPE')
    plt.legend()
    plt.savefig('PNG/[07]Absolute_Percent_Error_MAPE.png')
    plt.show()
    print(f"\n[log_14] Plot Absolute Percent Errors & Mean Absolute Percent Errors Done")
...
def compare(expectations_plot, predictions_plot):
    ...
    print(f"\n[log_15] Plot Predicted Values VS Actual Values")
    plt.figure(figsize=(20, 10))
    ...
    plt.savefig(f'PNG/[08]Predicted_Values_VS_Actual_Values.png')
    plt.show()
